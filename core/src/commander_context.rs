...
use crate::{steady_config, ActorIdentity, GraphLivelinessState, Rx, RxBundle, RxCore, SendSaturation, SteadyCommander, SteadyContext, Tx, TxBundle};
...
impl SteadyCommander for SteadyContext {
    ...
    fn avail_units<T: RxCore>(&mut self, this: &mut T) -> T::MsgSize {
    ...
    async fn take_async<T: RxCore>(&mut self, this: &mut T) -> Option<T::MsgOut> {
        if let Some((_d,r)) = this.shared_take_async().await {
            Some(r)
        } else {
            None
        }
