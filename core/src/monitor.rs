...
impl <const RXL: usize, const TXL: usize> LocalMonitor<RXL, TXL> {
    ...
    pub fn take_into_iter<'a, T>(& mut self, this: &'a  mut Rx<T>) -> impl Iterator<Item = T> + 'a {
        ...
        let avial = this.avail_units();
        this.local_index = if let Some(ref mut tel)= self.telemetry.send_rx {
            tel.process_event(this.local_index, this.channel_meta_data.id, avial)
        } else {
            MONITOR_NOT
        };
        //we only iterate the count of what was avail when we call ed this.
        this.shared_take_into_iter().take(avial)
    }
