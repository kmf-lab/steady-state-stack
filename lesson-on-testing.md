...
1. Level 1: Actor Simulation (IntoSimRunner)
...
1. Level 2: The StageManager (Integration)
...
#[test]
fn test_full_system_flow() -> Result<(), Box> {
...
1. Why This is Important: The "Wiring" Problem
...
1. Summary Best Practices
  Always provide a simulation branch: Every run function should have an if actor.use_internal_behavior check.
   Name your actors: Use .with_name("Processor") in your builder. The StageManager relies on these names to find the side-channels.
   Drop the StageManager Guard: The StageManager holds a lock on the graph's backplane. You must drop it (or let it go out of scope) before calling
   graph.block_until_stopped().
   Use i! in tests: Simulation relies on the is_running loop. If you don't use i!, you lose the ability to see why a test hung in the telemetry.
...
1. Verifying Output at the Edge
...
1. Testing with Suffixes (Troupes)
