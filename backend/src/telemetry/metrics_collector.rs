...
pub(crate) async fn run(context: SteadyContext
    ...
    let mut locked_option_svr = lock_if_some(&optional_server).await;
...
loop {
    ...
    //we do this here instead at the while because monitor is optional
        ...
        if let Some(nodes) = nodes {//only happens when we have new nodes
            send_structure_details(ident, &mut locked_option_svr, nodes).await;
        }
        send_data_details(ident, &mut locked_option_svr, &state, !is_shutting_down).await;
...
async fn full_frame_or_timeout(futures_unordered: &mut FuturesUnordered<BoxFuture<'_, (bool,Option<usize>)>>) -> Option<(bool,Option<usize>)> {
    select! {
        x = futures_unordered.next().fuse() => {
            //we need to find just one with a full frame of data
            x
        },
        _ = Delay::new(Duration::from_millis((config::TELEMETRY_PRODUCTION_RATE_MS>>1) as u64)).fuse() => {
            None
        }
    }
}
            ...
            for send_me in to_send {
               ...
               if let Some(DiagramData::NodeDef(seq, defs)) = Some(send_me.clone()) {
                   ...
                   //channels_out.iter().for_each(|x| info!("         channel out {:?} from {:?}",x.id,actor.id));
                    ...
                    let _ = c.send_async(ident, send_me, SendSaturation::IgnoreAndWait).await;
            }
    }
}

async fn send_data_details(ident: ActorIdentity, consumer: &mut Option<MutexGuard<'_, Tx<DiagramData>>>, state: &RawDiagramState, warn:bool) {
